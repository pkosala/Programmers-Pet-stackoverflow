...
df = df_with_code.withColumn("desc", udf_getDesc("body", "title"))
df = df.select('id', 'desc', 'code', 'title','tags')
df.show()
...
coeff_df = sqlContext.createDataFrame(coeff_dict)
...
insert_db(coeff_df, "coefficients", "overwrite")
